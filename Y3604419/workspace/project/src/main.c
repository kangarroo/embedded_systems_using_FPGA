...
static pthread_mutex_t system_mutex; //Main system mutex
	...
	while(1){
    	...
    	if(alarm_fl == 1){//Check Alarm is active
    		...
    		if(alarm_hr == hr_val && alarm_min == min_val && alarm_sec == sec_val){
    			...
    			sem_wait(&button_sem);
    			if(button_val == BUTTON_C){ //IF centre button pushed, snooze alarm for 5 mins
    				xil_printf("SNOOZE");
 ...
 */
void* stopwatch(void *data){
	if(pthread_mutex_lock(&system_mutex)){
	...
	char time[15];
	while(end == 0){
		sem_wait(&button_sem);// Wait for button press
		switch(button_val){
		 case BUTTON_N: //Lap
			//lcd_move(1,0);
			//lcd_print("                   ");
			...
			//Update current time with stopwatch time
			sec = sec+sw_sec;
			min = min+sw_min;
			hr = hr+sw_hr;
		 ...
		 case BUTTON_S: //Exit stopwatch
				...
				//Load time to RTC for clock mode
				XGpio_DiscreteWrite(&load_h,1,1);
				XGpio_DiscreteWrite(&load_m,1,1);
				XGpio_DiscreteWrite(&load_s,1,1);
				...
				XGpio_DiscreteWrite(&load_m,1,0);
				XGpio_DiscreteWrite(&load_s,1,0);
				button_val = 0;//Reset Button
				sw_exit = 1;//End run_stopwatch thread
				end = 1;//End stopwatch thread
				break;
	...
	if(pthread_mutex_unlock(&system_mutex)){
		xil_printf("MUTEX UNLOCK FAIL");
	}
	sw_exit = 0;
	pthread_exit(&thread4);//Close Stopwatch thread
	return 0;
}

/***********************************************
 * Run Stopwatch
 * This function runs the RTC for the stopwatch, and prints to the LCD
 * it is ended, when sw_exit flag is active.
 */
void* run_stopwatch(void *data){
	//LCD Tasks
	lcd_clear();
	...
	while(sw_exit == 0){
    	...
    	char time[10];
    	if(sw_sec != old_sec_val){//Check if seconds have changed
        	if(sw_sec == sw_min == 59 && sw_hr == 23){
        		sw_day++; //If end of day, increment Day counter
        	}
...
/****************************************
 * Set Time
 * This function allows the user to manually
 * set the time for the clock and alarm (depending on the state of the ALARM_SET flag.
 * ALARM SET (1) = Set Alarm
 * ALARM SET (0) = Set Time
 *
 * Button Inputs
 * North: Increment Time
 * East: Switch Time Unit ->
 * South: Decrement Time
 * West: Switch Time Unit <-
 * Centre: Set Time
 */
void* set_time(void *data){
	xil_printf("\nSet time\r\n");
	 ...
	 */
	if(alarm_set == 0){
		second = XIo_In32(XPAR_RTC_0_BASEADDR); //Get second value from RTC
		minute = XIo_In32(XPAR_RTC_0_BASEADDR | 0x00000004); //Get Minute value from RTC
		hour = XIo_In32(XPAR_RTC_0_BASEADDR | 0x00000008); //Get Hour value from RTC
	}else{
		second = alarm_sec;
		minute = alarm_min;
		hour = alarm_hr;
	}
	xil_printf("Current Time: s:%.2d m:%.2d h:%.2d\r\n",second,minute,hour);//Print current time
	char time[7];
	Xuint32 exit = 0;
	Xuint32 unit_switch = 0;//Variable for time unit
	lcd_clear();
	lcd_move(1,0);
	lcd_print("^^");//Unit indicator
	lcd_move(0,0);
	while(exit==0){
		...
		sem_wait(&button_sem);
		switch(button_val){
			case BUTTON_N://NORTH: INCREMENT UNIT
				if(unit_switch == 0){
					hour++;
					if(hour > 23){//Reset to 0 if overflow
						hour = 0;
					}
				} else if (unit_switch == 1){
					minute++;
					if(minute>59){//Reset to 0 if overflow
						minute = 0;
					}
				} else {
					second++;
					if(second>59){//Reset to 0 if overflow
						second = 0;
					}
				}
				break;
			case BUTTON_E://EAST: MOVE UNIT ->
				if(unit_switch<2){
					unit_switch++;
				...
				break;
			case BUTTON_S://SOUTH: DECREMENT UNIT
				if(unit_switch == 0){
					hour--;
					if(hour >= 63){//Go to max if underflow
						hour=23;
					}
				} else if (unit_switch == 1){
					minute--;
					if(minute >= 63){//Go to max if underflow
						minute = 59;
					}
				} else {
					second--;
					if(second >= 63){//Go to max if underflow
						second = 59;
					}
				}
				break;
			case BUTTON_W://WEST: MOVE UNIT <-
				if(unit_switch>0){
					unit_switch--;
					xil_printf("SWITCH: %d",unit_switch);
					if(unit_switch == 0){
				...
				break;
			case BUTTON_C://CENTRE: Set time
				if(alarm_set==0){
					//Write time to user accessible registers
					XGpio_DiscreteWrite(&load_h,1,1);
					XGpio_DiscreteWrite(&load_m,1,1);
					XGpio_DiscreteWrite(&load_s,1,1);
...
/***************************
 * Alarm Flash
 * Flashes the LEDs when the alarm is active
 */
void* alarm_flash(void *data){
	Xuint32 val = 1;
	while(alarm_end == 0){
		XGpio_DiscreteWrite(&LEDS,1,val);
		val  = val*2; //LEDs at 1,2,4,8 etc
		xil_printf("Alarm %d",val);
		if(val == 256){
			val = 1;//When max reached reset
		}
		sleep(50);
	}
	alarm_end = 0;
	XGpio_DiscreteWrite(&LEDS,1,0x00000000);//At end of alarm turn off ALL LEDs
...
/***********************************************
 * Xil Main
 * This thread sets up all of the GPIO Interrupts and starts
 * the run_clock thread.
 */
void* xil_main(void *data){
	...
	pthread_create(&thread1, &threadAttrs1, run_clock, NULL); //Start Run Clock Thread
...
/*********************
 * Main
 * Initialises all of the GPIO Components, Xilkernal Operating System,
 * Mutexes and Semaphores. Launches Xilkernal
 */
int main (void) {
